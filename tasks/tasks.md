...
- [x] **0.2** Uninstall `@tailwindcss/postcss` — it is a Tailwind v4 adapter installed alongside v3, creating a silent conflict. (`npm uninstall @tailwindcss/postcss`)
...
- [x] **0.4** Delete `src/types/interfaces.ts` — entirely commented-out dead code, superseded by `army.ts`.
- [x] **0.5** Delete or gitignore `src/styles/output.css` — compiled artefact not referenced by the app; causes noisy diffs.
- [x] **0.6** Fix `ArmyDetailTab.tsx` — remove its inline duplicate `createArmyUnit` function and import from `unitUtils.ts` instead.
- [x] **0.7** Fix `any[]` prop types in `BattleForgeTab.tsx` and `NewArmyModal.tsx` — replace with `Unit[]`.
- [x] **0.8** Fix `setArmies` prop signature in `BattleForgeTab` — change from `(armies: Army[]) => void` to `Dispatch<SetStateAction<Army[]>>` to allow functional update patterns.
- [x] **0.9** Normalize line endings — run `git add --renormalize .` to resolve the mixed CRLF/LF state across the repo.
