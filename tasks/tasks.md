...
| Multi-resolution / responsive layout | The 390×844 phone shell is the primary design target. Desktop works even if imperfect. Not a current priority. |
...
- [ ] **0.1** Delete `postcss.config.mjs`. Keep only `postcss.config.js` (CJS, Tailwind v3).
- [ ] **0.2** Uninstall `@tailwindcss/postcss` — it is a Tailwind v4 adapter installed alongside v3, creating a silent conflict. (`npm uninstall @tailwindcss/postcss`)
- [ ] **0.3** Add `darkMode: 'class'` to `tailwind.config.js`. Without this, every `dark:` utility across all components is dead and the theme system does not work.
- [ ] **0.4** Delete `src/types/interfaces.ts` — entirely commented-out dead code, superseded by `army.ts`.
- [ ] **0.5** Delete or gitignore `src/styles/output.css` — compiled artefact not referenced by the app; causes noisy diffs.
- [ ] **0.6** Fix `ArmyDetailTab.tsx` — remove its inline duplicate `createArmyUnit` function and import from `unitUtils.ts` instead.
- [ ] **0.7** Fix `any[]` prop types in `BattleForgeTab.tsx` and `NewArmyModal.tsx` — replace with `Unit[]`.
- [ ] **0.8** Fix `setArmies` prop signature in `BattleForgeTab` — change from `(armies: Army[]) => void` to `Dispatch<SetStateAction<Army[]>>` to allow functional update patterns.
- [ ] **0.9** Normalize line endings — run `git add --renormalize .` to resolve the mixed CRLF/LF state across the repo.
...
- [ ] **1.4 Reference tab — real content**
...
- [ ] **2.1 Space Marines core roster**
  - Populate `src/data/units.json` with ~20–30 Space Marines units.
...
- [ ] **3.1 Soft army composition warnings**
  ...
  - Display as a banner or badge — never a hard block.
